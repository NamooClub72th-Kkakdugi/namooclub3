							...
							<ul class="list-group">
								<c:forEach var="community" items="${joinCommunities}">
									<li class="list-group-item"><span class="badge"></span>
										<h4>
										<c:if test="${presCommunity.manager.email == email}">
										<span class="label label-warning">관리자</span>
										</c:if>
											<a href="${ctx}/club/clubList.do?comNo=${community.comNo}">${community.name}&nbsp;(회원수 : ${presCommunity.members.size()} 클럽수 : ${community.clubs.size()})</a>
										</h4>
										...
										<c:choose>
										<c:when test="${presCommunity.manager.email == email}">
